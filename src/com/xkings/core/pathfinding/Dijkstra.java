 ...
 * <p/>
 * An implementation of Dijkstra's single-source shortest path algorithm. The algorithm takes as input a directed graph with non-negative edge costs and a
 * source node, then computes the shortest path from that node to each other node in the graph.
 * <p/>
 * The algorithm works by maintaining a priority queue of nodes whose priorities are the lengths of some path from the source node to the node in
 * question. At each step, the algortihm dequeues a node from this priority queue, records that node as being at the indicated distance from the source,
 * and then updates the priorities of all nodes in the graph by considering all outgoing edges from the recently- dequeued node to those nodes.
 * <p/>
 * In the course of this algorithm, the code makes up to |E| calls to decrease-key on the heap (since in the worst case every edge from every node will
 * yield a shorter path to some node than before) and |V| calls to dequeue-min (since each node is removed from the prioritiy queue at most once). Using a
 * Fibonacci heap, this gives a very good runtime guarantee of O(|E| + |V| lg |V|).
 * <p/>
...
public final class Dijkstra {
    /**
     * Given a directed, weighted graph G and a source node s, produces the distances from s to each other node in the graph. If any nodes in the graph
     * are unreachable from s, they will be reported at distance +infinity.
     *
    ...
    public static class DijkstraResult<T> {
        ...
        /**
         * Maintain a map from nodes to their distances. Whenever we expand a node for the first time, we'll put it in here.
         */
