 ...
 * Time: 5:05 PM <br>
 */
public class SmoothShapeRenderer {
    /**
     * ****************************************************************************
    ...
    public SmoothShapeRenderer() {
        this(5000, null);
    }

    public SmoothShapeRenderer(int maxVertices, ShaderProgram shader) {
        if (Gdx.graphics.isGL20Available()) {
            ImmediateModeRenderer20 renderer20 = new ImmediateModeRenderer20(maxVertices, false, true, 0);
            if (shader != null) {
                renderer20.setShader(shader);
            }
            renderer = renderer20;
        } else {
    ...
    /**
     * Sets the projection matrix to be used for rendering. Usually this will be set to {@link com.badlogic.gdx.graphics.Camera#combined}.
     *
     * @param matrix
     */
    ...
    /**
     * Starts a new batch of shapes. All shapes within the batch have to have the type specified. E.g. if {@link ShapeType#Point} is specified, only call
     * #point().
     * <p/>
     ...
     */
    public void line(float x, float y, float z, float x2, float y2, float z2, float thickness) {
        if (currType != ShapeType.Filled && currType != ShapeType.Line)
            throw new GdxRuntimeException("Must call begin(ShapeType.Line) or begin" + "(ShapeType.Filled)");
        checkDirty();
        if (thickness == 1) {
            checkFlush(2);
            renderer.color(color.r, color.g, color.b, color.a);
            ...
            Vector2[] rect = Collision.getRectOverLine(x, y, x2, y2, thickness);
            checkFlush(3);

            renderer.color(color.r, color.g, color.b, color.a);
            renderer.vertex(rect[0].x, rect[0].y, 0);
            renderer.color(color.r, color.g, color.b, color.a);
            renderer.vertex(rect[2].x, rect[2].y, 0);
            renderer.color(color.r, color.g, color.b, color.a);
            renderer.vertex(rect[1].x, rect[1].y, 0);
            ...
            renderer.vertex(rect[2].x, rect[2].y, 0);
            renderer.color(color.r, color.g, color.b, color.a);
            renderer.vertex(rect[3].x, rect[3].y, 0);
        }
    }
     ...
     */
    public void line(float x, float y, float x2, float y2, float thickness) {
        line(x, y, 0, x2, y2, 0, thickness);
    }

    /**
     * Draws a line in the x/y plane. The {@link ShapeType} passed to begin has to be {@link ShapeType#Line}. The line is drawn with 2 colors interpolated
    ...
    /**
     * Draws a rectangle in the x/y plane. The x and y coordinate specify the bottom left corner of the rectangle. The {@link ShapeType} passed to begin
     * has to be {@link ShapeType#Filled} or {@link ShapeType#Line}.
     */
    ...
    /**
     * Draws a box. The x, y and z coordinate specify the bottom left front corner of the rectangle. The {@link ShapeType} passed to begin has to be
     * {@link ShapeType#Line}.
     */
     ...
     */
    public void x(float x, float y, float radius, float thickness) {
        if (currType != ShapeType.Line) throw new GdxRuntimeException("Must call begin(ShapeType.Line)");
        line(x - radius, y - radius, x + radius, y + radius, thickness);
        line(x - radius, y + radius, x + radius, y - radius, thickness);
    }
    ...
    public void circle(float x, float y, float radius) {
        circle(x, y, radius, Math.max(1, (int) (12 * (float) Math.cbrt(radius))));
    }
    ...
    /**
     * Calls {@link #cone(float, float, float, float, float, int)} by estimating the number of segments needed for a smooth circular base.
     */
    ...
    /**
     * Draws a polygon in the x/y plane. The vertices must contain at least 3 points (6 floats x,y). The {@link ShapeType} passed to begin has to be
     * {@link ShapeType#Line}.
     *
     * @param vertices
     */
    ...
    /**
     * Draws a polyline in the x/y plane. The vertices must contain at least 2 points (4 floats x,y). The {@link ShapeType} passed to begin has to be
     * {@link ShapeType#Line}.
     *
     * @param vertices
     */
