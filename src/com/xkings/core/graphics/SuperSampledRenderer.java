...
/**
 * Renderer wrapper that writes to larger buffer and them scales it down. User: Tomas <br>
 * Date: 7/19/13 <br> Time: 3:57 AM <br>
 */
public class SuperSampledRenderer implements Renderable {
     ...
     * <ul> <li>Scale factor greater than 1 increases quality, but decreases
     * performance.</li> <li>Scale factor smaller than 1 decreases quality, but increases
     * performance.</li> </ul>
     *
     * @param renderer to be wrapped around
    ...
    public SuperSampledRenderer(Renderable renderer, float factor) {
        ...
        frameBufferObject = new FrameBuffer(RGB565, (int) (graphics.getWidth() * factor),
                (int) (graphics.getHeight() * FACTOR), false);
    ...
    public void render() {
        ...
        spriteBatch.begin();
        spriteBatch.draw(frameBufferRegion, 0, 0, graphics.getWidth(),
                graphics.getHeight());
        spriteBatch.end();
    }
}