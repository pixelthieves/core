...
public final class DirectedGraph<T> implements Iterable<T> {
    ...
    /**
     * Given a start node, destination, and length, adds an arc from the start node to the destination of the length. If an arc already existed, the
     * length is updated to the specified value. If either endpoint does not exist in the graph, throws a NoSuchElementException.
     *
    ...
    /**
     * Removes the edge from start to dest from the graph. If the edge does not exist, this operation is a no-op. If either endpoint does not exist, this
    ...
    /**
     * Given a node in the graph, returns an immutable view of the edges leaving that node, as a map from endpoints to costs.
     *
